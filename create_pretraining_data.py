...
flags.DEFINE_string(
    "output_file", "/usr/idip/idip/liuan/project/Multimodal/bert-master/tmp/tf_examples.tfrecord",
    "Output TF example file (or comma-separated list of files).")

# flags.DEFINE_string("vocab_file", None,
#                     "The vocabulary file that the BERT model was trained on.")
flags.DEFINE_string("vocab_file", "/usr/idip/idip/liuan/project/Multimodal/bert-master/data/cased_L-12_H-768_A-12/vocab.txt",
                    "The vocabulary file that the BERT model was trained on.")
...
def write_instance_to_example_files(instances, tokenizer, max_seq_length,
  ...
  for (inst_index, instance) in enumerate(instances):
    ...
    if inst_index < 20:   # 对前20个训练样本进行打印
...
def create_instances_from_document(
  ...
  if rng.random() < short_seq_prob:     # 产生一个随机数如果小于short_seq_prob则产生一个较短的训练序列
    ...
    if i == len(document) - 1 or current_length >= target_seq_length:
      if current_chunk:
        ...
        if len(current_chunk) >= 2:
          a_end = rng.randint(1, len(current_chunk) - 1)  # 从current_chunk中随机选出一个文档作为句子

        tokens_a = []
        for j in range(a_end):
          tokens_a.extend(current_chunk[j])  # 将截至文档之前的文档都加入到tokens_a
        ...
        if len(current_chunk) == 1 or rng.random() < 0.5:  # 候选集只有一句的情况则随机抽取句子作为句子2；或以0.5的概率随机抽取句子作为句子2
          ...
          random_document = all_documents[random_document_index]  # 随机找一个文档作为截至文档
          random_start = rng.randint(0, len(random_document) - 1)    # 随机找一个初始文档
          for j in range(random_start, len(random_document)):
            tokens_b.extend(random_document[j])                   # 将随机文档加入token_b
        ...
        truncate_seq_pair(tokens_a, tokens_b, max_num_tokens, rng)   # 对两个句子进行长度裁剪
...
def main(_):
  ...
  for input_pattern in FLAGS.input_file.split(","):
    input_files.extend(tf.gfile.Glob(input_pattern)) # input文件路径的string串
