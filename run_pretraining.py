...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(      # Estimator要求返回一个EsitimatorSpec对象
          mode=mode,
          loss=total_loss,
          train_op=train_op,
      ...
      def metric_fn(masked_lm_example_loss, masked_lm_log_probs, masked_lm_ids,
        ...
        masked_lm_log_probs = tf.reshape(masked_lm_log_probs,
                                         [-1, masked_lm_log_probs.shape[-1]])    # 概率矩阵转化成[batch_size*max_pred_Pre_seq, vocab_size]
        masked_lm_predictions = tf.argmax(
            masked_lm_log_probs, axis=-1, output_type=tf.int32)      # 取最大值的位置为输出
...
def get_masked_lm_output(bert_config, input_tensor, output_weights, positions,
  ...
  input_tensor = gather_indexes(input_tensor, positions)    # [batch_size*max_pre_pre_seq, hidden_size]

  with tf.variable_scope("cls/predictions"):
    # We apply one more non-linear transformation before the output layer.
    # This matrix is not used after pre-training.
    with tf.variable_scope("transform"):
      input_tensor = tf.layers.dense(       # 传入一个全连接层，输出shape [batch_size*max_pred_seq, hidden_size]
          input_tensor,
    ...
    one_hot_labels = tf.one_hot(             # [batch_size*max_pred_pre_seq, vocab_size]
        label_ids, depth=bert_config.vocab_size, dtype=tf.float32)   # label id 转 one hot

    # The `positions` tensor might be zero-padded (if the sequence is too
    # short to have the maximum number of predictions). The `label_weights`
    # tensor has a value of 1.0 for every real prediction and 0.0 for the
    # padding predictions.
    per_example_loss = -tf.reduce_sum(log_probs * one_hot_labels, axis=[-1])   # [batch_size*max_pre_per_seq]
